@testset "Chapman-Enskog" begin
    @testset "Model" begin
        ...
        @test dynamic_viscosity(model, 200.)/1e-6 ≈ 7.6848 rtol=1e-2                # NIST value
        model_db = ChapmanEnskogModel("methane"; ref="Poling et al. (2001)")
        @test dynamic_viscosity(model, NaN, 300.) ≈ dynamic_viscosity(model_db, NaN, 300.)
        ...
        # @test dynamic_viscosity(model_Neufeld, 200.)/1e-6 ≈ 7.6848 rtol=1e-2    
