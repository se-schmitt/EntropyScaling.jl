@testitem "GCES" begin
    ...
    @testset "Thermal conductivity" begin
        @testset "pure" begin
            ...
            model = GCES(("ethane", ["CH3" => 2]))
            T, p = 250.0, 10e6
            @test thermal_conductivity(model, p, T) ≈ 0.125625 rtol=1e-3
            ...
            model = GCES(comp, eos)
            T, p = 450.0, 4e6
            @test thermal_conductivity(model, p, T) ≈ 0.084622 rtol=1e-3

            # benzene
            model = GCES(("benzene", ["aCH" => 6]))
            T, p = 400.0, 10e6
            @test thermal_conductivity(model, p, T) ≈ 0.115090 rtol=1e-3
            ...
            model = GCES(comp, eos)
            T, p = 350.0, 10e6
            @test thermal_conductivity(model, p, T) ≈ 0.113760 rtol=1e-3

            # cyclohexane
            model = GCES("cyclohexane")
            T, p = 400.0, 10e6
            @test thermal_conductivity(model, p, T) ≈ 0.101814 rtol=1e-3
        end
