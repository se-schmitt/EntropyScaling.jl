...
@testset "RefpropRES" begin
    ...
    @testset "Pure Viscosity" begin
        ...
        model_ace = RefpropRES(SingleFluid("acetone"),Dict(
        ...
        model_cyc = RefpropRES(SingleFluid("cyclopentane"),Dict(
        ...
        model_13but = RefpropRES("1,3-butadiene")
        @test viscosity(model_13but,9.259e3,220.)/1e-6 ≈ 424.7 rtol=1e-4

        # Martinek et al. (2025)
        model_r11_martinek = RefpropRES("R11"; ηref="Martinek et al. (2025)")
    ...
    @testset "Mixtures Viscosity" begin
        ...
        model_mix = RefpropRES(MultiFluid(["decane","methane"]),Dict(
