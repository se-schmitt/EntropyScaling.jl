...
@testset "RefpropRES" begin
    @testset "Pure Dynamic Viscosity" begin
        ...
        model_ace = RefpropRESModel(SingleFluid("acetone"),Dict(
            DynamicViscosity() => [ni_ace, ξ_ace, σ_ace, ε_ace]
        ))
        @test dynamic_viscosity(model_ace,1.3922e4,281.005)/1e-6 ≈ 371.6 rtol=1e-4
        ...
        model_cyc = RefpropRESModel(SingleFluid("cyclopentane"),Dict(
            DynamicViscosity() => [ni_cyc, ξ_cyc, σ_cyc, ε_cyc]
        ))
        @test dynamic_viscosity(model_cyc,1.4244e4,273.15)/1e-6 ≈ 567.0 rtol=1e-4

        # Test database
        model_ace_db = RefpropRESModel("acetone")
        @test dynamic_viscosity(model_ace,1e5,300.) ≈ dynamic_viscosity(model_ace_db,1e5,300.)

        model_cyc_db = RefpropRESModel("cyclopentane")
        @test dynamic_viscosity(model_cyc,1e5,300.) ≈ dynamic_viscosity(model_cyc_db,1e5,300.)
    end

    @testset "Mixtures Dynamic Viscosity" begin
        ...
        model_mix = RefpropRESModel(MultiFluid(["decane","methane"]),Dict(
            DynamicViscosity() => [ni_mix, ξ_mix, σ_mix, ε_mix]
        ))
        @test dynamic_viscosity(model_mix,3.565407e6,289.996,[0.8680,1-0.8680])/1e-6 ≈ 925.02 rtol = 1e-5

        # Test database
        model_mix_db = RefpropRESModel(["decane","methane"])
        @test dynamic_viscosity(model_mix, 1e5, 300., [.5,.5]) ≈ dynamic_viscosity(model_mix_db, 1e5, 300., [.5,.5])
    end 
