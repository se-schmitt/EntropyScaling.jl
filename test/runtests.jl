...
# EOS models
eos_but = PCSAFT("n-butane")
eos_mix1 = PCSAFT(["benzene","hexane"])
eosparam_mix2 = (;dipole=[2.88,0.0],epsilon=[232.99,271.63],sigma=[3.2742,3.4709],Mw=[58.08,119.38],segment=[2.7447,2.5038])
eos_mix2 = PCPSAFT(["acetone", "chloroform"]; userlocations=param_mix2)
eos_mix3 = PCSAFT(["toluene","heptane"])
...
@testset "fit [old]" begin
    # Viscosity
    (α_η, ηˢ, s_η) = fit_entropy_scaling(eos_but, exp_η[:,1], exp_η[:,2], exp_η[:,3], "vis")
    @test all(isapprox(α_η, [0.0,-9.490597,9.747817,-1.279090,0.3666153]; atol=1e-5))

    # Thermal conductivity
    (α_λ, λˢ, s_λ) = fit_entropy_scaling(eos_but, exp_λ[:,1], exp_λ[:,2], exp_λ[:,3], "tcn"; i_fit=[1,1,1,1,1])
    @test all(isapprox(α_λ, [3.532197, 116.820497, -99.444059, 24.246002, 0.58713]; atol=1e-5))

    # Self-diffusion coefficient
    (α_D, Dˢ, s_D) = fit_entropy_scaling(eos_but, exp_D[:,1], exp_D[:,2], exp_D[:,3], "dif"; i_fit=[0,0,0,1,1])
    @test all(isapprox(α_D, [0.0, 0.0, 0.0, -3.573438, -0.99224]; atol=1e-5))
end

@testset "fit inf dil [old]" begin
    # Infinite dilution diffusion coefficient
    solute = Dict(:M=>0.08618, :Tc=>519.33427, :pc=>3.542718e6)
    (α_D_inf, D_infˢ, s_D_inf) = fit_entropy_scaling(eos_but, exp_D_inf[:,1], exp_D_inf[:,2], exp_D_inf[:,3], "dif"; solute=solute, i_fit=[0,0,0,1,1])   
    @test all(isapprox(α_D_inf, [0.0, 0.0, 0.0, -1.153974, -0.5866215]; atol=1e-5))
end

@testset "call [old]" begin
    @test call_entropy_scaling(eos_but,  [323.],   [ 602.4253281], "vis"; α=α_η_but)[1] ≈ 1.9203575445e-4 rtol=1e-3
    @test call_entropy_scaling(eos_but,  [323.],   [ 602.4253281], "tcn"; α=α_λ_but)[1] ≈ 0.11980321582 rtol=1e-3
    @test call_entropy_scaling(eos_but,  [323.],   [ 602.4253281], "dif"; α=α_D_but)[1] ≈ 6.6470158395e-9 rtol=1e-3
    @test call_entropy_scaling(eos_mix1, [294.7],  [ 740.7881963], "tcn"; x=[.5 .5;], α=α_λ_mix1)[1] ≈ 0.1338505 rtol=1e-3
    @test call_entropy_scaling(eos_mix2, [298.15], [1151.7720905], "selfdif"; x=[.5 .5;], α=α_D_mix2, difcomp=1)[1] ≈ 2.872998e-9 rtol=1e-3
    @test call_entropy_scaling(eos_mix3, [308.15], [ 741.0969526], "mutdif"; x=[.5 .5;], α=α_Ð_mix3)[1] ≈ 3.3341968265e-9 rtol=1e-3
end